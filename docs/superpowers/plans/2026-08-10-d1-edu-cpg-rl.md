# 智元 D1 EDU（ZSL-1）CPG-RL 平地走路 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 把 task4 在 Go2 上驗證過的 CPG-RL 論文標準版移植到智元 D1 EDU（點足版 ZSL-1），訓出平地走路權重，且 observation 只用實機 SDK 拿得到的量。

**Architecture:** 以 zsibot 的 ZSL-1 URDF 真實質量/慣量為準，手寫 menagerie 風格 MJCF（`d1_edu.xml` + `scene.xml` + `scene_mjx.xml`）。動作 12 維經 CPG 產生腳掌偏移，再用「home 姿態數值 Jacobian 求逆」轉成關節角，交給 MJCF 內建位置伺服（kp=80/kd=1，與原廠 demo 一致）。訓練在 Colab MJX，推論在本機 CPU。
...
- 質量、慣量、關節原點、關節軸、關節限位**一律照 URDF 原值**，不得參考 Go2 代換。
- 關節限位（rad）：abad `[-0.4887, 0.4887]`、hip `[-1.1519, 2.967]`、knee `[-2.723, -0.602]`。
- knee 關節軸為 `0 -1 0`（**與 Go2 的 `0 1 0` 相反**）。
- home 關節角 `HOME3 = [0.0, -0.94, -1.80]`、`HOME12 = HOME3 * 4`。
...
| 檔案 | 責任 |
|---|---|
| `task6/model/d1_edu/d1_edu.xml` | 機器人本體 MJCF：body 樹、慣量、關節、碰撞、致動器、感測器、home keyframe |
| `task6/model/d1_edu/scene.xml` | 本機 CPU 場景（含 skybox / 地板材質 / 燈光） |
| `task6/model/d1_edu/scene_mjx.xml` | MJX 訓練場景（同上，`<option>` 走 MJX 參數） |
| `task6/model/d1_edu/meshes/*.STL` | 17 個視覺網格 |
| `task6/model/d1_edu/LICENSE` | zsibot BSD-3 授權原文 |
| `task6/model/d1_edu/SOURCE.md` | 資產來源與 commit 記錄 |
...
## Task 1: task6 骨架與模型資產

**Files:**
- Create: `task6/model/d1_edu/meshes/` （17 個 STL，從 zsibot repo 複製）
- Create: `task6/model/d1_edu/LICENSE`
- Create: `task6/model/d1_edu/SOURCE.md`
- Create: `task6/inference/__init__.py`（空檔，讓 tests 可 import）
- Test: `task6/tests/test_assets.py`

**Interfaces:**
- Consumes: 無
- Produces: `task6/model/d1_edu/meshes/{BASE,FL_ABAD,FL_HIP,FL_KNEE,FL_FOOT,FR_*,RR_*,RL_*}_LINK.STL` 共 17 檔
...
Expected: FAIL — `缺少網格: [...]`（17 個全缺）

- [ ] **Step 3: 取得資產並放到定位**

```bash
cd /home/huang/rbtdog_sim
mkdir -p task6/model/d1_edu/meshes task6/inference task6/tests task6/docs task6/notebooks task6/weights task6/outputs
touch task6/inference/__init__.py

TMP=$(mktemp -d)
git clone --depth 1 https://github.com/zsibot/genisom_model.git "$TMP/genisom"
cp "$TMP/genisom/zsl-1/meshes/"*.STL task6/model/d1_edu/meshes/
cp "$TMP/genisom/LICENSE"           task6/model/d1_edu/LICENSE
git -C "$TMP/genisom" rev-parse HEAD > /tmp/genisom_commit.txt
cat /tmp/genisom_commit.txt
rm -rf "$TMP"
ls task6/model/d1_edu/meshes | wc -l   # 應為 17
```

- [ ] **Step 4: 寫 SOURCE.md**

Create `task6/model/d1_edu/SOURCE.md`（把 Step 3 印出的 commit 填進 `<commit>`）:
...
cd /home/huang/rbtdog_sim
git add task6/
git commit -m "feat(task6): 匯入 D1 EDU (ZSL-1) 模型資產與來源記錄"
```

---

## Task 2: `d1_edu.xml` 本體 MJCF（關卡 1）

**Files:**
- Create: `task6/model/d1_edu/d1_edu.xml`
- Test: `task6/tests/test_model.py`

**Interfaces:**
- Consumes: `task6/model/d1_edu/meshes/*.STL`（Task 1）
- Produces: MJCF 名稱契約，後續所有任務依賴 ——
  - body：`base`、`{LEG}_abad`、`{LEG}_hip`、`{LEG}_knee`、`{LEG}_foot`
  - joint：`{LEG}_abad_joint`、`{LEG}_hip_joint`、`{LEG}_knee_joint`
  - actuator：`{LEG}_abad`、`{LEG}_hip`、`{LEG}_knee`（順序 FL, FR, RL, RR × abad, hip, knee）
  - 足端碰撞 geom：`FL`、`FR`、`RL`、`RR`
  - site：`imu`
...
Expected: 全部 ERROR / FAIL — 找不到 `d1_edu.xml`

- [ ] **Step 3: 寫 MJCF**

Create `task6/model/d1_edu/d1_edu.xml`。慣量全部抄自 `ZSL-1.urdf`，
...
<mujoco model="d1_edu">
  ...
  <default>
    <default class="d1">
      ...
      <default class="hip">
        <joint axis="0 1 0" range="-1.1519 2.967"/>
        <general ctrlrange="-1.1319 2.947"/>
      </default>
      <default class="knee">
      ...
      <default class="collision">
        <geom group="3"/>
        <default class="foot">
          <geom type="sphere" size="0.028" pos="-0.003 0 -0.006" priority="1"
                condim="6" solimp="0.015 1 0.031" friction="0.8 0.02 0.01"/>
        </default>
      </default>
    </default>
  </default>
  ...
  <worldbody>
    <body name="base" pos="0 0 0.2914" childclass="d1">
      <camera name="track" pos="0.85 -1.47 0.92" xyaxes="0.866 0.5 0 -0.171 0.296 0.94" mode="trackcom"/>
      <inertial pos="0.0054244583 -0.00036330247 0.0060908623" mass="6.2683759"
        fullinertia="0.027359128 0.084984679 0.099223551 -0.000016710628 0.0008748801 -0.000013572831"/>
      <freejoint/>
      ...
      <body name="FL_abad" pos="0.17449 0.062 0">
        ...
        <body name="FL_hip" pos="0 0.097412 0">
          <inertial pos="-0.00334388 -0.042909 -0.0216077" mass="1.50165"
            fullinertia="0.0069465757 0.0047644743 0.0032561192 0.00024600926 -0.00044260461 0.0014434247"/>
          ...
          <body name="FL_knee" pos="0 0 -0.2">
            <inertial pos="0.0076906034 0 -0.091770469" mass="0.20733416"
              fullinertia="0.0015906767 0.0016230863 0.000049932184 0.0000000002681 0.000030406347 0.000000018098293"/>
            <joint name="FL_knee_joint" class="knee"/>
            <geom mesh="FL_KNEE_LINK" class="visual"/>
            <geom type="capsule" fromto="0 0 -0.02 0 0 -0.20" size="0.020" class="collision"/>
            <body name="FL_foot" pos="0 0 -0.21366">
              <inertial pos="-0.0045073 0 -0.00867028" mass="0.063194610"
                fullinertia="0.000020429232 0.000021658445 0.000023037914 -0.0000000026626 0.0000018517964 -0.0000000012479232"/>
      ...
      <!-- ================= FR ================= -->
      <body name="FR_abad" pos="0.17449 -0.062 0">
        <inertial pos="-0.027074453 0.015458828 -0.0000094167931" mass="0.45409769"
          fullinertia="0.000251027 0.0004874808 0.00054102144 -0.000153060 -0.0000000321747 -0.0000000208886"/>
        <joint name="FR_abad_joint" class="abad"/>
        <geom mesh="FR_ABAD_LINK" class="visual"/>
        <body name="FR_hip" pos="0 -0.097399 0">
          <inertial pos="-0.0034244521 0.0427302652 -0.022325654" mass="1.50799"
            fullinertia="0.0071458222 0.0049559736 0.0032698207 -0.00025057726 -0.000457345 -0.0014871635"/>
          ...
          <body name="FR_knee" pos="0 0 -0.2">
            ...
            <geom type="capsule" fromto="0 0 -0.02 0 0 -0.20" size="0.020" class="collision"/>
            <body name="FR_foot" pos="0 0 -0.21366">
              <inertial pos="-0.0045073 0 -0.00867028" mass="0.063194610"
                fullinertia="0.000020429232 0.000021658445 0.000023037914 -0.0000000026626 0.0000018517964 -0.0000000012479232"/>
      ...
      <!-- ================= RL ================= -->
      <body name="RL_abad" pos="-0.17449 0.062 0">
        <inertial pos="0.027074453 -0.015458828 -0.0000094167931" mass="0.45409769"
          fullinertia="0.000251027 0.0004874808 0.00054102144 -0.000153060 0.0000000321747 0.0000000208886"/>
        <joint name="RL_abad_joint" class="abad"/>
        <geom mesh="RL_ABAD_LINK" class="visual"/>
        <body name="RL_hip" pos="0 0.097412 0">
          <inertial pos="-0.00334388 -0.042909 -0.0216077" mass="1.50165"
            fullinertia="0.0069465757 0.0047644743 0.0032561192 0.00024600926 -0.00044260461 0.0014434247"/>
          ...
          <body name="RL_knee" pos="0 0 -0.2">
            ...
            <geom type="capsule" fromto="0 0 -0.02 0 0 -0.20" size="0.020" class="collision"/>
            <body name="RL_foot" pos="0 0 -0.21366">
              <inertial pos="-0.0045073 0 -0.00867028" mass="0.063194610"
                fullinertia="0.000020429232 0.000021658445 0.000023037914 -0.0000000026626 0.0000018517964 -0.0000000012479232"/>
      ...
      <body name="RR_abad" pos="-0.17449 -0.062 0">
        ...
        <body name="RR_hip" pos="0 -0.0974 0">
          ...
          <body name="RR_knee" pos="0 0 -0.2">
            ...
            <geom type="capsule" fromto="0 0 -0.02 0 0 -0.20" size="0.020" class="collision"/>
            <body name="RR_foot" pos="0 0 -0.21366">
              <inertial pos="-0.0045073 0 -0.00867028" mass="0.063194610"
                fullinertia="0.000020429232 0.000021658445 0.000023037914 -0.0000000026626 0.0000018517964 -0.0000000012479232"/>
  ...
  <keyframe>
    <key name="home"
         qpos="0 0 0.2914 1 0 0 0  0 -0.94 -1.80  0 -0.94 -1.80  0 -0.94 -1.80  0 -0.94 -1.80"
         ctrl="0 -0.94 -1.80  0 -0.94 -1.80  0 -0.94 -1.80  0 -0.94 -1.80"/>
  </keyframe>
</mujoco>
```
...
```bash
cd /home/huang/rbtdog_sim
git add task6/model/d1_edu/d1_edu.xml task6/tests/test_model.py
git commit -m "feat(task6): D1 EDU 本體 MJCF + 與 URDF 對帳測試（關卡 1）"
```
...
**Files:**
- Create: `task6/model/d1_edu/scene.xml`
- Create: `task6/model/d1_edu/scene_mjx.xml`
- Create: `task6/inference/d1_model.py`
- Modify: `task6/tests/test_model.py`（附加站姿測試）

**Interfaces:**
- Consumes: `d1_edu.xml` 的名稱契約（Task 2）
- Produces:
...
def test_d1_model_constants_are_single_source_of_truth():
    ...
    assert d1_model.HOME3 == pytest.approx([0.0, -0.94, -1.80])
    assert d1_model.HOME12 == pytest.approx([0.0, -0.94, -1.80] * 4)
...
def test_home_pose_is_statically_stable():
    """關卡 2：home 姿態靜置 2 秒，機身 z 漂移 < 1 cm。"""
    import d1_model
    ...
    assert abs(z1 - z0) < 0.01, (
        f"站姿 2 秒內機身 z 由 {z0:.4f} 漂到 {z1:.4f}（漂移 {abs(z1-z0)*100:.1f} cm）。"
        "kp=80/kd=1 為原廠值不得擅改；若持續失敗請回報數據給使用者決定。"
    )
    assert float(d.qvel[2]) == pytest.approx(0.0, abs=0.05), "站定後垂直速度應趨近 0（不應持續抖動）"
```
...
```xml
<mujoco model="d1_edu scene">
  <include file="d1_edu.xml"/>
...
Create `task6/model/d1_edu/scene_mjx.xml`（內容與 `scene.xml` 相同 —— `d1_edu.xml` 的 `<option>`
已經是 MJX 參數，兩者差異保留給未來需要時分化）:

```xml
<mujoco model="d1_edu scene (mjx)">
  <include file="d1_edu.xml"/>
...
```python
"""D1 EDU（ZSL-1）模型與常數的單一事實來源。

所有 task6 腳本與 Colab notebook 的常數都必須與本檔一致，否則訓練與推論會對不起來。
數值出處：
  - 幾何/質量/慣量/限位：zsibot/genisom_model 的 zsl-1/urdf/ZSL-1.urdf
...
_MODEL_DIR = Path(__file__).resolve().parents[1] / "model" / "d1_edu"
SCENE = str(_MODEL_DIR / "scene.xml")
SCENE_MJX = str(_MODEL_DIR / "scene_mjx.xml")

# ---- 機構 ----
LEGS = ["FL", "FR", "RL", "RR"]        # 與 SDK motorCmd 的 SoA 索引順序一致（見 task6/docs 對照表）
HOME3 = np.array([0.0, -0.94, -1.80])  # abad, hip, knee；hip 正負號與 Go2 相反（knee 軸為 -y）
HOME12 = np.tile(HOME3, 4)

# ---- 控制 ----
CTRL_DT = 0.02      # 50 Hz，落在 SDK 建議的 20~50 Hz 內
SIM_DT = 0.004
KP, KD = 80.0, 1.0  # 原廠 demo 值；模擬與實機共用，不得擅改
...
```bash
cd /home/huang/rbtdog_sim
git add task6/model/d1_edu/scene.xml task6/model/d1_edu/scene_mjx.xml \
...
def test_leg_ik_consts_home_foot_is_below_hip():
    """home 姿態下腳應在髖正下方約 0.257 m。"""
    m = d1_model.make_model()
    f0s, jinvs = cpg_d1.leg_ik_consts(m)
    assert f0s.shape == (4, 3)
    assert jinvs.shape == (4, 3, 3)
    for k in range(4):
        assert abs(f0s[k][0]) < 0.02, f"腿 {k} 的腳未在髖正下方 (x={f0s[k][0]:.3f})"
        assert -0.30 < f0s[k][2] < -0.24, f"腿 {k} 髖到腳距離異常 (z={f0s[k][2]:.3f})"


def test_ik_jacobian_moves_foot_in_requested_direction():
    """把腳往 +x 推 2 cm，正向運動學算回來的位移應吻合（線性化誤差 < 3 mm）。"""
    import mujoco
...
(a) home 機身高度 (b) armature/damping 假設 (c) `D_STEP`/`G_C` 對 D1 腿長不合。
不得自行連續調參後宣稱通過。
...
**Files:**
- Create: `task6/notebooks/cpg_rl_d1_colab.ipynb`

**Interfaces:**
- Consumes: `task6/model/d1_edu/*`（notebook 內用 `git clone` 取本專案或手動上傳）
- Produces: `cpg_rl_d1_params.pkl`（Brax PPO params）
...
Create `task6/notebooks/cpg_rl_d1_colab.ipynb`，Cell 1（markdown）:

```markdown
# CPG-RL 訓練：智元 D1 EDU（ZSL-1）· MJX · Colab GPU

移植自 task4 的 Go2 論文標準版。與 Go2 版的差異：

| 項目 | Go2（task4）| D1（本檔）|
|---|---|---|
| home 關節角 | `[0, 0.9, -1.8]` | `[0, -0.94, -1.80]`（knee 軸為 -y，hip 正負號相反）|
...
SCENE = "rbtdog_sim/task6/model/d1_edu/scene_mjx.xml"
...
HOME3_np = np.array([0.0, -0.94, -1.80])
...
model.save_params("cpg_rl_d1_params.pkl", params)
try:
    from google.colab import files; files.download("cpg_rl_d1_params.pkl")
except Exception as e:
    print("左側檔案面板右鍵下載 cpg_rl_d1_params.pkl。", e)
```
...
```bash
cd /home/huang/rbtdog_sim
git add task6/notebooks/cpg_rl_d1_colab.ipynb
git commit -m "feat(task6): Colab MJX 訓練 notebook（73 維 obs、kp80/kd1、payload 5kg、IMU 雜訊 DR）"
```
...
訓完把 `cpg_rl_d1_params.pkl` 下載放到 `task6/weights/`。
...
**Interfaces:**
- Consumes: `d1_model.*`、`cpg_d1.*`、`obs_d1.build_obs`、`task6/weights/cpg_rl_d1_params.pkl`
...
用法：
  ...
  conda run --no-capture-output -n rbtdog python task6/inference/local_infer_d1.py \
      --params task6/weights/cpg_rl_d1_params.pkl --secs 20 --video --push
"""
import argparse
import os
import sys
...
def run(args) -> dict:
    ...
    if frames:
        ...
        out = OUT_DIR / f"infer_d1{tag}.mp4"
        iio.mimsave(str(out), frames, fps=25, codec="libx264")
        print("[影片]", out)
    return res
...
```
Expected: 印出 `[result] ...` 一行，產生 `task6/outputs/infer_d1.mp4`，且不拋例外。
...
拿到 `task6/weights/cpg_rl_d1_params.pkl` 後執行：
```bash
cd /home/huang/rbtdog_sim
conda run --no-capture-output -n rbtdog python task6/inference/local_infer_d1.py \
    --params task6/weights/cpg_rl_d1_params.pkl --secs 20 --video
conda run --no-capture-output -n rbtdog python task6/inference/local_infer_d1.py \
    --params task6/weights/cpg_rl_d1_params.pkl --secs 20 --video --push
```
把兩份指標與影片交給使用者。
...
**Files:**
- Create: `task6/docs/D1_EDU_規格與模型對照.md`
...
| 連續上階高度 | 16 cm |
| 額定負載 | 5 kg |
| 電池 | 43.2 V / 4.6 Ah，續航 1~2 小時 |

## 2. 模擬模型採用值

| 項目 | 值 | 出處 |
|---|---|---|
| 總質量 | 15.19 kg | URDF 17 個 link 之和 |
| 力矩上限 | **28 N·m** | URDF `effort`（官網 48 為峰值，取保守值）|
| kp / kd | **80 / 1** | 官方 demo `lowlevel_demo.py:68-73` |
| home 關節角 | `[0, −0.94, −1.80]` | 解 `foot_x = 0`，髖到腳 0.2574 m |
| home 機身高 | 0.2914 m | 髖到腳 + 足球心偏移 + 半徑 |
| armature / damping | 0.01 / 0.1 | **URDF 未提供，為假設值**，經關卡 2、3 驗證 |

## 3. ⚠️ SDK 腿序：官方文件自相矛盾，尚未定案
...
模擬採 `LEGS = ["FL", "FR", "RL", "RR"]`（模擬內部自洽，不影響訓練）。

**sim2real 前必做**：連上實機後，用 LowLevel 對單一腿下小幅度位置指令，
目視確認是哪一條腿在動，四條全部確認後把結論寫回本節，再進行部署。**不得憑文件推測。**

## 4. `motorCmd` 是 SoA 佈局

```cpp
float q_des_abad[4];  float q_des_hip[4];  float q_des_knee[4];
```

模擬的 12 維關節向量是 **per-leg AoS**（`[FL_abad, FL_hip, FL_knee, FR_abad, ...]`），
送進 SDK 前必須重排：`q_des_abad[k] = q[3*k+0]`、`q_des_hip[k] = q[3*k+1]`、`q_des_knee[k] = q[3*k+2]`
（前提是第 3 節的腿序已驗證）。

## 5. observation 與實機感測對照

因官方 FAQ 明示「Highlevel 与 Lowlevel 不可以同时使用」，自訂 policy 只能用 LowLevel 的
`getQuaternion` / `getRPY` / `getBodyAcc` / `getBodyGyro` / `getMotorState` / `haveMotorData`。
...
**刻意排除**：機身線速度（`getBodyVelocity` 只存在於 HighLevel，不可並用）。
reward 仍用模擬真值速度，因為 reward 只在模擬計算。
...
`task6/inference/d1_model.py` 與 `task6/notebooks/cpg_rl_d1_colab.ipynb` 各有一份常數
（Colab 不 import 本地模組，屬刻意重複）。**改一邊就必須改另一邊**，否則訓練與推論會對不起來。
...
```markdown
# task6 — 智元 D1 EDU（ZSL-1）CPG-RL 走路

把 task4 在 Go2 上驗證過的 CPG-RL 論文標準版移植到智元 D1 EDU 點足版。
...
| 項目 | Go2 | D1 EDU |
|---|---|---|
| home 關節角 | `[0, 0.9, -1.8]` | `[0, -0.94, -1.80]`（knee 軸 -y）|
...
# 3. 訓練（Colab GPU）：上傳 notebooks/cpg_rl_d1_colab.ipynb，
...
`docs/D1_EDU_規格與模型對照.md` 第 3 節 —— **官方文件的腿序自相矛盾，必須用實機驗證後才能部署。**
```
