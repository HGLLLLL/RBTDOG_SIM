...
- `OBS_DIM = 69`、`ACT_DIM = 12`。（原設計 73 維含 4 維觸地布林，關卡 3 實測證明無可用訊號後移除，見下）
- **不得使用 `TAU_CONTACT` 或任何力矩/位置誤差門檻判觸地**：關卡 3 實測顯示這台輪足版的擺動相膝力矩(p95 8.76 N·m)與位置誤差(p95 0.144 rad)皆**大於**站立相(p05 1.55 N·m / 0.017 rad)，因為 0.9 kg 的輪子在擺動時的加速需求蓋過承重訊號。膝角速度與 Jacobian 反推足端力也試過，前者重疊、後者僅約 85% 準確。使用者決定移除該 4 維。
...
| `task6/inference/d1_model.py` | **單一事實來源**：路徑、`LEGS`、`HOME3/HOME12`、CPG 常數、`OBS_DIM`、`NOMINAL_HEIGHT`、`make_model()` |
| `task6/inference/cpg_d1.py` | CPG 動力學 + 動作解碼 + IK 常數 + 關節目標角（純函式，無 I/O） |
| `task6/inference/obs_d1.py` | 69 維 observation 組裝 |
...
- Produces:
  ...
  - `d1_model.OBS_DIM: int = 69`、`d1_model.ACT_DIM: int = 12`
...
def foot_geom_ids(m: mujoco.MjModel) -> list[int]:
    ...
    輪子鎖死當圓腳，功能上等同足端，故沿用 foot_* 命名供 IK 與量測引用。
    """
...
- Produces: `cpg_openloop_d1.rollout(secs, omega, mu, video) -> dict`
  （鍵：`dist`, `theory`, `fell`, `foot_lift`, `final_z`）

這是整個計畫最重要的早期關卡：**不花任何 GPU 時數**就能一次驗證 MJCF、home 姿態、
IK Jacobian、位置伺服增益、CPG 常數五件事。

> **關卡 3 已於 2026-08-10 由控制端完成實測並判定通過**，以下判準與預期值即為實測結果。
> 首次執行時曾因理論值算錯（漏算 trot 每週期有兩個站立相）而誤判為「打滑假通過」，
> 修正後確認名目摩擦下前進量與理論值只差 1%。詳見本任務 Step 2 的說明。

- [ ] **Step 1: 寫腳本**

Create `task6/inference/cpg_openloop_d1.py`：開迴路 CPG rollout（不含 RL）。
必要元素：

- 用 `d1_model.make_model()` 建模型、`mujoco.mj_resetDataKeyframe(m, d, 0)` 起始於 home
- 先用 `d1_model.HOME12` 站穩 0.5 秒再開始擺動
- 每個控制步：`cpg_d1.cpg_step` → `cpg_d1.joint_targets` → `d.ctrl[:] = q_des` → `mj_step` × n_sub
  （**位置伺服在 MJCF 內，不得有軟體 PD**）
- 量測：前進距離、是否跌倒（`cpg_d1.w2b` 求重力，`grav[2] > -0.4` 判翻倒）、
  FL 輪 geom 的 z 範圍（抬腳量）、末端機身高
- **理論前進量**必須算對：trot 每個週期有**兩個**站立相（對角腿輪流），
  機身每半週期前進一個步幅 `2·D_STEP·fx`，故
  `theory = 2 * (2 * D_STEP * fx) * omega * secs`，其中 `fx = 2*(mu - MU_MIN)/(MU_MAX - MU_MIN) - 1`
- `--video` 時輸出 `task6/outputs/openloop_cpg.mp4`（鏡頭跟隨機身）
...
**通過標準（依實測校準）**：
- 前進距離 / 理論值 落在 **0.85 ~ 1.20**（實測名目摩擦 0.8 時為 **0.99**）
- 不跌倒
- FL 抬腳量 **0.05 ~ 0.12 m**（實測 0.084 m）

  ⚠️ 這個區間刻意與 Go2 版的「0.03~0.06」不同。`G_C = 0.08` 是**指令**的離地量，
  量到 0.084 代表腿把軌跡追得很準，是**通過訊號而非缺陷**。用 Go2 的區間會誤判。

**已知限制（不列為失敗，但要記錄）**：摩擦係數超過 1.0 後步態會崩潰
（實測 1.5 → −0.85 m、3.0 → +0.19 m）。訓練的摩擦 DR 範圍是 [0.3, 1.0]，
實測 0.3/0.5/0.8/1.0 分別為 5.28/5.20/4.57/4.12 m，**全範圍內皆正常且不跌倒**。
根因是站立相足端速度為正弦（0 → 0.905 m/s）而機身近似等速，相位內必然有滑動；
低摩擦時滑動廉價，高摩擦時腳抓地反而與機身互相對抗。這是此 CPG 公式的固有特性。

**未通過就停下來回報**，附上實際數值與影片；候選原因依序為
**(a) 鎖死輪子的接觸摩擦**(b) `D_STEP`/`G_C` 對這台的腿長不合
(c) home 機身高度 (d) armature/damping 假設。不得自行連續調參後宣稱通過。
...
Run: `conda run --no-capture-output -n rbtdog python -m pytest task6/tests/test_cpg_d1.py -v`
Expected: 全部 PASS
...
```bash
cd /home/huang/rbtdog_sim
git add task6/inference/cpg_openloop_d1.py task6/tests/test_cpg_d1.py
# 注意：.gitignore 有全 repo 的 *.mp4 政策（0 個 mp4 被追蹤），影片留在本機不入庫
git commit -m "feat(task6): 開迴路 CPG 驗證通過（關卡 3）"
```

---

## Task 6: `obs_d1.py` — 69 維觀測
...
- Produces:
  - `build_obs(d, c, cmd, last_a) -> np.ndarray`（shape (69,) float32）
...
(2) 腳觸地布林——關卡 3 實測證明這台輪足版沒有可用的觸地訊號：
    0.901 kg 的輪子讓擺動相的膝力矩與位置誤差都**大於**站立相，訊號方向是反的。
...
| 力矩上限 | 23.7 / knee 45.43 | 28 全關節 |
| obs | 76 維 | 69 維（移除機身線速度與觸地布林）|
| 觸地判定 | 腳掌世界高度 | 移除（無可用訊號）|
...
| §6 六道關卡 | 關卡 1→Task 2、2→Task 3、3→Task 5（**已實測通過**）、4/5→Task 7、6→Task 8 |
| §7 分工 | Task 7 Step 9、Task 8 Step 4 |
| §8 YAGNI | 全計畫無地形、無輪足、無實機部署 |
| §9 已知未決事項 | 腿序→Task 9；kp/kd 穩定性→Task 3 Step 5；力矩上限→Task 7 訓練後回報 |
...
**型別一致性**：`LEGS` / `HOME3` / `HOME12` / `OBS_DIM` / `D_STEP_Y` / `NOMINAL_HEIGHT` 只在 `d1_model.py` 定義一次；
