...
## 附錄：v3.2 原廠命令週期產生器（spec §9；2026-09-15 下午核准）

### Task 7: `ref_extract.cmd_cycle` → `outputs/ref_cmd_cycles.json`
- 以 fl 擺動起點（`swing_events31`）切週期、100 點正規化、平均 des／q／輪 τ；MJCF 序、控制器座標；輪 τ 正 = 向前滾。
- 測試：`tests/test_ref_extract_v31.py` 加：json 四個鍵、形狀 (100,12)/(100,4)、freq 2.0–2.7、平移 ABAD 偏移 ptp ≥ 0.5 rad。
- 驗證：`python ref_extract.py` 後 json 存在；commit。

### Task 8: env v3.2（`rl_env_v3.py`）
- 載入 json 為常數 `CYC[dir]`；`cycle_offsets(phi, cmd, A, ref)` → (12,) 關節偏移、(4,) 輪速偏移、hz；`REF["step_gen"]="cycle"|"kin"`（turn／lat 各自）。
- `step()`：踏步族 u>0 時 q_des = clip(STANCE + u·(cycle_off·amp) + (1−u)·kin) …實作上直接以 `s4` 加權混合週期偏移與 v3.1 足端 IK 結果；ARC 仍走 v3.1。
- obs cpg 區 8 格改放 ABAD／膝命令偏移；`err_barrier` 分關節（ABAD 0.6）。
- 測試更新：`test_env_shapes…`、新增 `test_cycle_offsets_mirror_and_amp`。

### Task 9: G0 v3.2 掃描
- `g0_v3.py` 加 `--cyc-amp-turn`、`--cyc-amp-lat`、`--step-gen`；LAT 10 s roll std 門檻；TURN 10 s 站滿。
- 掃 cyc_amp_turn {0.5, 0.7, 1.0}、平移 recenter {站姿, 絕對}；退路規則照 spec §9。結果進 spec §9.1。

### Task 10: 影片、HANDOFF、commit
- `render_g0_v3.py` 重出 `outputs/g0_v32_baseline_small.mp4`（標題加「v3.2 原廠命令週期」），送使用者確認。
