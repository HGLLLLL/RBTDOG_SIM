...
| 6 | obs 維度 | 76 | **69** | §5 |
| 7 | 觸地判定 | 腳掌世界高度 < 3 cm | **移除**（實測無可用訊號）| §5.3.1 |
...
| 指令 | 3 | ✅ 自產 | 保留 |
| 上一動作 | 12 | ✅ 自存 | 保留 |
| 腳觸地布林 | 4 | ❌ 無接觸感測 | **移除**（見 5.3.1）|
| CPG 狀態 | 24 | ✅ 自算 | 保留 |

**obs = 3 + 3 + 12 + 12 + 3 + 12 + 24 = 69 維。**

#### 5.3.1 為什麼觸地那 4 維最後被移除（關卡 3 實測結論）

原設計打算用「膝關節力矩 > 門檻」取代 Go2 版的「腳掌世界高度」判定，因為力矩兩邊都算得到。
**關卡 3 的開迴路實測推翻了這個假設**（以 MuJoCo 接觸為真值，摩擦 0.8，1600 個樣本）：

| 候選訊號 | 站立相 | 擺動相 | 結論 |
|---|---|---|---|
| 膝關節力矩 \|τ\| | p05 = 1.55 N·m | p95 = 8.76 N·m | ❌ 擺動相反而更大 |
| 膝位置誤差 \|q_des − q\| | p05 = 0.017 rad | p95 = 0.144 rad | ❌ 擺動相反而更大 |
| 膝關節角速度 | p95 = 4.40 rad/s | p05 = 0.26 rad/s | ❌ 重疊 |
| Jacobian 反推足端 Fz | 中位 −55 N | 中位 +15 N | ⚠️ 尾部重疊，門檻 −20 N 約 85% 準確 |

**根因**：這台是輪足版，足端是 **0.901 kg 的輪**（點足版只有 0.063 kg）。
擺動腿要在約 0.25 秒內把輪子加上 0.765 kg 的小腿甩起 8 cm，所需力矩與造成的追蹤延遲
**蓋過了站立相的承重訊號**；而站立時腿接近垂直、力臂小、膝力矩本來就低。
這是輪足構型特有的，點足版不會發生。

使用者決定**直接移除這 4 維**（obs 73 → 69），理由：CPG 相位（sinθ/cosθ 已在 obs 中）
在平地上幾乎等同觸地資訊，額外價值低；而 85% 準確度的二元特徵會往 obs 注入噪訊。
下一輪做地形訓練時再重新評估。
...
| 4 | Smoke test | `obs.shape == (69,)`；reward 有限、非 NaN | 使用者（Colab）|
