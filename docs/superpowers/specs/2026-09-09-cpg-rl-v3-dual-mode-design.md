...
> **⚠️ 勘誤（2026-09-15）**：發現 1（差速轉向要位置環）與發現 3（平移靠輪胎側滑、不可行）都錯了。前者是力矩權限不夠（原廠輪 kd 0.1 幾乎是力矩控制、後輪會負力矩煞車），後者是產生器把另一側兩腿釘死（原廠四腿都踏）；此外 v3.0 的相位偏移把週期分數直接當弧度加（0.5 週期變成 0.08 週期），本節所有踏步掃描結論都受影響，「kp250 做不到原廠 2.5／2.1 Hz」在平移已被推翻（2.1 Hz 可行）。v3.0 從未訓練；由 `2026-09-15-cpg-rl-v3.1-unified-kinematic-generator-design.md` 取代 §1 與本節。
