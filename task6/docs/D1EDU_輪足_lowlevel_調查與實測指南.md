...
> ### ✅ 實機實測結果（2026-08-10，序號 SN01PW8626B0012）
>
> 用 `shm_probe` 即時畫面 + 手扳關節實測，**確認這台走 SHM enum 那套（FR 開頭）**：
>
> | leg 索引 | 實體腿 | 狀態 |
> |---|---|---|
> | `leg0` | **右前 FR** | ✅ 已實測確認（手抬右前腿，leg0 的 knee.p 跟著變） |
> | `leg1` | 左前 FL | 推論（依 enum 順序，尚未逐一實測） |
> | `leg2` | 右後 RR | 推論 |
> | `leg3` | 左後 RL | 推論 |
>
> → SHM enum（`LEG_FRONT_RIGHT=0`）是對的，**不是**高層 API 那套。
> 之後寫控制程式以 leg0=FR 為準。建議有空再實測一條後腿（leg2/leg3）鎖定前後順序。
>
> **另一個關鍵實測**：SHM 讀到的關節角度**與官方 URDF 限位對不上**
> （abad 讀 ±0.98 vs URDF ±0.49；knee 讀 ≈0 vs URDF −2.72~−0.60）。
> 代表馬達編碼器的零點/正負/慣例與 URDF（MJCF 基礎）不同，
> **sim2real 時必須在中間補一層「每關節 offset + 正負校正」**。
> 此外白板讀取實測 ~880 Hz、firefly 非 root 即可讀、IMU 四元數與時戳皆正常。
