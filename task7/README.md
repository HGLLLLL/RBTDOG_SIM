...
| `docs/現場操作卡_首次連線與偵察.md` | ★ **上機時照著做**。WiFi 連線、路由、跑偵察、卡住怎麼辦。設計成離線可用 |
...
**照 `docs/現場操作卡_首次連線與偵察.md` 做。** 那份是離線可用的逐步指令，
含連線、路由、卡住的處置。核心就一行：

```bash
bash task7/realbot/recon_d1max.sh          # WiFi 模式（預設）
bash task7/realbot/recon_d1max.sh --wired  # 有線模式
```

零風險（全程唯讀），跑完會印自動判讀。之後照主文件第 9 節的待確認清單往下做。

⚠️ 連上狗的 WiFi 之後電腦會斷網（wlan0 不支援同時連兩個網路），
所以操作卡設計成**離線自足**——出發前先看過一遍。
