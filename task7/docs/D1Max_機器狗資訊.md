...
| onnxruntime / torch | **無** |
| CPU 親和性 | 8 核，可用 **0–6**；**`cpu7` 被核心隔離保留給運控**（`isolcpus=7 nohz_full=7`，與 D1 EDU 相同），推論程式**絕對不能跑在 cpu7** |
...
| 內部光達 | 前 `192.168.1.102`、後 `192.168.2.102`（獨立網段） |
...
| 3 | **`cpu7` 被核心隔離給運控獨占**，推論程式絕不能跑在 cpu7 | 三機型對照／obs盤點 |
