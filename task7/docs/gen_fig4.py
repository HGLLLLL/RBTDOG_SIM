...
# ------------------------------------------------------------------ 標題
text(L, 40, "智元三款四足機器狗：運算平台、SDK 能力與單顆馬達控制可行性", 20, weight="bold")
text(L, 64, "硬體規格為實機實測（cat /proc/device-tree/model、lscpu、free -h、dmesg）；"
            "SDK 與控制介面為官方文件＋二進位符號分析＋實機偵察", 12.5, fill=GREY)
...
for i, (nm, kind, spec) in enumerate(heads):
    ...
    text(x + 16, HY + 27, nm, 19, weight="bold")
    text(x + 16 + len(nm) * 11 + 22, HY + 27, kind, 13.5, fill=GREY)
    text(x + 16, HY + 50, spec, 13)

# ------------------------------------------------------------------ 甲：運算平台
SY = HY + HH + 22
text(L, SY + 20, "（甲）", 15, weight="bold")
text(L, SY + 44, "運算平台", 16, weight="bold")
text(L, SY + 66, "實機實測", 11.5, fill=GREY)
...
for i, boards in enumerate(plats):
    ...
    for (nm, sub, rows, big) in boards:
        ...
        text(x + 14, y + 24, nm, 14.5, font=MONO if mono else SERIF, weight="bold")
        text(x + 14, y + 43, sub, 12, fill=GREY)
        for k, r in enumerate(rows):
            text(x + 14, y + 64 + k * 20, r, 12.5)
        y += bh + 10
    verdict = ["單一算力平台", "★ 兩塊獨立高算力板", "主控板＋轉發板（非第二算力板）"][i]
    text(x + 14, SY + SH + 4, verdict, 13,
...
text(L, TY + 20, "（乙）", 15, weight="bold")
text(L, TY + 44, "SDK", 16, weight="bold")
text(L, TY + 66, "可寫深度", 16, weight="bold")
text(L, TY + 88, "官方介面", 11.5, fill=GREY)
...
for i in range(3):
    ...
    for k, lab in enumerate(levels):
        ...
        depth = 4 - k                        # L4→4, L1→1
        ok = depth <= reach[i]
        box(x, yy, COLW, LH - 6, fill=FILL_D if ok else WHITE,
            sw=1.2 if ok else 0.9, dash=None if ok else "4 4")
        text(x + 12, yy + 19, lab, 12.5, fill=INK if ok else GREY)
    ...
    if reach[i] < 4:
        ...
        lbl = "官方 SDK 止步於此"
        lw = len(lbl) * 11 + 10
        box(x + COLW - 8 - lw, stop_y - 22, lw, 18, fill=WHITE, stroke=WHITE, sw=0, rx=0)
        text(x + COLW - 12, stop_y - 8, lbl, 11.5, anchor="end", weight="bold")
...
for i in range(3):
    ...
    box(x, ry, COLW, 28, fill=WHITE, sw=0.9)
    text(x + 12, ry + 19, rd, 12.5, fill=GREY)

sdkname = ["mc_sdk::zsl_1w::HighLevel　UDP 50 Hz",
           "robot_sdk::SDKClient　UDP :8082",
           "高層 TCP ＋ 底層 ROS 1 話題"]
for i in range(3):
    text(CX[i] + 12, ry + 52, sdkname[i], 12, font=MONO, fill=GREY)

# ------------------------------------------------------------------ 丙：單顆馬達控制
VY = ry + 74
line(L, VY - 10, W - L, VY - 10, sw=1.4)
text(L, VY + 20, "（丙）", 15, weight="bold")
text(L, VY + 44, "單顆馬達", 16, weight="bold")
text(L, VY + 66, "控制", 16, weight="bold")
...
cards = [
    ...
    ("官方不提供", "極可能做得到", "/dev/shm/joint_cmd　16 軸 × 5 欄位",
     ["介面已定位、結構已完整解出；",
      "寫入尚未測試（需 root 權限）"], 2),
    ("★ 官方提供", "做得到", "ROS 1 話題　rt/lowcmd",
     ["官方文件所載；本團隊尚未實測。",
      "底層 SDK 必須安裝 ROS 1"], 1),
]
CH = 146
for i, (off, verd, path, ev, star) in enumerate(cards):
    x = CX[i]
    strong = (i == 2)
    box(x, VY, COLW, CH, fill=FILL_L if strong else WHITE, sw=1.6 if strong else 1.2)
    text(x + 14, VY + 24, off, 13.5, weight="bold" if strong else "normal",
         fill=INK if strong else GREY)
    text(x + 14, VY + 48, verd, 17, weight="bold")
    text(x + 14, VY + 70, path, 12.5, font=MONO)
    ln = ev
    for k, seg in enumerate(ln):
        text(x + 14, VY + 92 + k * 17, seg, 11.5, fill=GREY)
    # 星等
    sx = x + COLW - 14
    text(sx, VY + 24, "●" * star + "○" * (3 - star), 13, anchor="end")

text(CX[0] + 14, VY + CH + 20,
     "● ● ●  實機驗證　　● ●  介面已解、待驗證　　●  文件記載、未實測", 12, fill=GREY)

# ------------------------------------------------------------------ 圖說
cy = VY + CH + 52
line(L, cy - 20, W - L, cy - 20, sw=1.4)
text(L, cy, "圖 4 · 三款機型比較。", 13, weight="bold")
text(L + 118, cy,
     "（甲）運算平台：D1 Max 是三台中唯一的雙高算力板；D1 MaxPro 雖是大狗，"
     "算力平台與小狗 D1 EDU 同為單顆 RK3588。", 13)
text(L, cy + 20,
     "（乙）三台的「讀」幾乎全開，差別在「寫」——D1 EDU 與 D1 Max 的官方 SDK 只到姿態動作層，"
     "送不了關節角度或力矩；只有 D1 MaxPro 開放到單顆馬達。", 13)
text(L, cy + 40,
     "（丙）D1 EDU 與 D1 Max 官方雖不提供，但機上存在共享記憶體介面可繞過 SDK："
     "D1 EDU 已實機驗證三種控制模式，D1 Max 結構已解出、僅差寫入未測。", 13)
text(L, cy + 62,
     "註：D1 MaxPro 官方文件的 SSH 帳號為 jetson，但實測主控板為 Firefly RK3588，"
     "兩者不一致，機型批次差異待向原廠確認。", 11.5, fill=GREY)
