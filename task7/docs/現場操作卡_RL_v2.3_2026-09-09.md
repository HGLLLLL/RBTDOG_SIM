...
0. **場地短 → 停車靠 Enter**：走路中（含淡入中）按空行 Enter 就進 3 秒淡出，**淡出那一刻 policy 自動切回開迴路 A**，
   所以按下去之後的 3 秒就是 trip17 驗過的那個淡出（≈ 0.3 m）。模擬裡 v2.3 比 A 慢 15%（0.29 vs 0.34 m/s），
   同樣的 `--walk-max` 走的距離不會比 trip17 遠；`--walk-max` 是忘了按的保險，會自動進淡出。
