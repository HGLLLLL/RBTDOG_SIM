...
# 名目機身高度（m）。
# `_KIN` 是 HOME 的**純運動學**值；`NOMINAL_HEIGHT` 是實際落地站定後量到的。
NOMINAL_HEIGHT_KIN = 0.4914
NOMINAL_HEIGHT = 0.4589      # `cpg_walk_max.py --stand` 實測（4 s 收斂後）

# ★ 位置伺服的靜態撓度（m）。kp 有限，要撐住 41 kg 就必須留追蹤誤差，
#   所以指令的足端基準會比實際站立的足端**低** STATIC_SAG。
#   = NOMINAL_HEIGHT_KIN − NOMINAL_HEIGHT，實測 32.5 mm。
#
#   ⚠️ 這個值會被 CPG 的抬腿指令直接吃掉：實際離地 = g_c − STATIC_SAG。
#      不補償的話 g_c 小於 32.5 mm 時腿根本不會離地，而且**四個診斷指標全是乾淨的**。
#      詳見 `cpg_max.foot_targets` 的說明。
#   ⚠️ 它與 kp 綁在一起。改 KP3 就必須重跑 `--stand` 重新量這個值。
STATIC_SAG = NOMINAL_HEIGHT_KIN - NOMINAL_HEIGHT
