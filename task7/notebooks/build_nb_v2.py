...
看實測步率（60M 超過 3 小時就砍 `TIMESTEPS` 到 40M 重跑）→ 下載 `cpg_rl_max_v2_params.pkl`
...
# 指標怎麼讀（每控制步平均）：roll/pitch 越小越好（基準 roll 平均約 1.5°）；exec f/r 越接近 1 越好；
...
print("已存 cpg_rl_max_v2_params.pkl → 下載放 task7/weights/，本機（rbtdog 環境）跑：")
