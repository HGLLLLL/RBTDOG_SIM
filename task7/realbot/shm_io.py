...
class Shm:
    ...
    def imu(self) -> dict:
        ...
        ⚠️ **四元數的順序是 `xyzw`。旁證已經很充分，但正式的 A/B 對照實驗仍未做。**

        2026-08-25 的初判依據是「取樣當下加速度顯示重力在 +Z、x/y ≈ 0，所以機身水平；
        用 xyzw 解得 roll −0.12° / pitch −1.61°（與加速度自洽），
        用 wxyz 解得 roll −30.6°（矛盾）」—— 那是一次**剛好水平的巧合取樣**，
        不是刻意做的平放實驗。

        2026-08-27 M7／M8 十餘趟承重實測補上了強得多的旁證：
          - 承重站立 roll +0.6~+1.9° / pitch −1.0~−2.2°，與眼睛看到的一致
          - 抬腿時 roll 往抬起那一側增加，方向對
          - ★ 最有力的一條：M8 拿 roll/pitch 把足端量測**旋轉到重力對齊座標系**後，
            **兩個不同 roll 值下支撐腳都讀到 0 mm 離地**。順序若錯，roll 會是垃圾，
            這個修正不可能在兩種姿態下同時歸零。

        ⚠️ 但這些都是「解出來的姿態自洽」，**不是** xyzw／wxyz 的直接對照。
