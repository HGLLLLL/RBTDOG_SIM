"""70 維觀測層（2026-09-08 起：last_action 12→14，加 body sway）。
...
def test_layout_sums_to_dim():
    assert sum(d for _, d in obs_max.OBS_LAYOUT) == obs_max.OBS_DIM == 70
...
def test_build_obs_dim_and_dtype(level_data):
    o = obs_max.build_obs(level_data, _c(), np.array([0.15, 0.0]), np.zeros(obs_max.ACT_DIM))
    assert o.shape == (70,)
...
def test_gravity_is_down_when_level(level_data):
    ...
    o = obs_max.build_obs(level_data, _c(), np.array([0.0, 0.0]), np.zeros(obs_max.ACT_DIM))
...
def test_cmd_and_last_action_land_where_expected(level_data):
    ...
    a = np.arange(obs_max.ACT_DIM, dtype=float)
...
def test_wrong_size_is_rejected(level_data):
    ...
    with pytest.raises(AssertionError):
        obs_max.build_obs(level_data, _c(), np.array([0.1, 0.0, 0.0]), np.zeros(obs_max.ACT_DIM))
    with pytest.raises(AssertionError):
...
def test_obs_excludes_base_linear_velocity(level_data):
    ...
    o = obs_max.build_obs(d, _c(), np.array([0.0, 0.0]), np.zeros(obs_max.ACT_DIM))
    d.qvel[0:3] = 0.0
...
def test_local_infer_action_mapping_matches_env_module():
    """ω/sway 的映射範圍必須與訓練端（rl_env_max）同值。
    ...
    import local_infer_max
    import rl_env_max
    assert (local_infer_max.OMEGA_MIN, local_infer_max.OMEGA_MAX) == \
        (rl_env_max.OMEGA_MIN, rl_env_max.OMEGA_MAX)
    assert (local_infer_max.SWAY_MAX, local_infer_max.SWAY_SLEW) == \
        (rl_env_max.SWAY_MAX, rl_env_max.SWAY_SLEW)


def test_dummy_reproduces_open_loop_baseline_bit_exactly():
    """★ `--dummy` 跑出來的指標必須與開迴路 A 基準**逐位相同**。

    基準步態在這個動作空間裡是一個固定動作（mux/muy/ω 用 atanh 反推、sway=0），
    ...
    args = argparse.Namespace(params="", dummy=True, secs=6.0, vx=0.15, wz=0.0,
                              video=False, scene=None, perturb=1, compare=False)
    with contextlib.redirect_stdout(io.StringIO()):
        got = local_infer_max.run(args)
        want = cw.rollout(gait="walk_a", secs=6.0, kp3=A["kp3"], kd3=A["kd3"],
                          kd_wheel=A["wheel_kd"], z_sag=A["z_sag"], quiet=True)
    for k in ("speed_travel", "bounce", "pitch_mean", "support", "yaw",
              "net_roll", "min_lift", "height", "fell", "exec_front"):
